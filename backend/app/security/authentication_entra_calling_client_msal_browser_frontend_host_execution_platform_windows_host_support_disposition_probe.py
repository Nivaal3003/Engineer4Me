"""Pure Step 240 Windows host-support disposition proof.
...
_CANONICAL_MODULE_NAME = (
    ...
    "execution_platform_windows_host_support_disposition_probe"
)
if __name__ == "__main__":
    ...
    if (
        ...
        raise RuntimeError("Step 240 CLI module identity cannot be bound safely")
    sys.modules[_CANONICAL_MODULE_NAME] = _current_module


AUTHORIZATION_TYPE = (
    "engineer4me_microsoft_entra_calling_client_msal_browser_frontend_host_"
    "execution_platform_windows_host_support_disposition_authorization"
)
EVIDENCE_TYPE = (
    "engineer4me_microsoft_entra_calling_client_msal_browser_frontend_host_"
    "execution_platform_windows_host_support_disposition_evidence"
)
RECEIPT_TYPE = (
    "engineer4me_microsoft_entra_calling_client_msal_browser_frontend_host_"
    "execution_platform_windows_host_support_disposition_receipt"
)
SCHEMA_VERSION = 1
AUTHORIZATION_SOURCE = "engineer4me_step240_closed_package_installer"
EVIDENCE_SOURCE = (
    "engineer4me_step240_closed_package_installer_read_only_windows_host_inventory"
)
SOURCE = "engineer4me_pure_windows_host_support_disposition_probe"
SCOPE = (
    "exact_step239_chain_plus_read_only_registry64_cim_and_commercial_esu_"
    "windows_host_support_disposition"
)
...
# Exact registry-edition/CIM-SKU correlations accepted as ordinary Windows
# client editions.  LTSC/LTSB, IoT, Server, evaluation, Cloud, and virtual
# edition mappings are intentionally absent and therefore fail closed.
_CLIENT_EDITION_SKU = {
    ("Core", "Core", 101): "home",
...
class WindowsHostSupportDispositionError(ValueError):
    """Sanitized Step 240 proof failure."""
...
def _load_canonical_json(document: bytes) -> dict[str, Any]:
    if type(document) is not bytes or not document or len(document) > MAX_CLI_ENVELOPE_BYTES:
        raise ValueError("Step 240 envelope size is invalid")
    value = json.loads(
        document,
    ...
    if type(value) is not dict or _canonical(value) != document:
        raise ValueError("Step 240 envelope is not exact canonical JSON")
    return value
...
def _validate_authorization(value: object) -> dict[str, Any]:
    authorization = _exact_object(value, _AUTHORIZATION_KEYS, "Step 240 authorization")
    expected: dict[str, object] = {
        "approved_step239_accepted_state_manifest_sha256": STEP239_ACCEPTED_STATE_MANIFEST_SHA256,
        "approved_step239_canonical_receipt_sha256": STEP239_CANONICAL_RECEIPT_SHA256,
        "approved_step239_package_manifest_sha256": STEP239_PACKAGE_MANIFEST_SHA256,
    ...
    for name, expected_value in expected.items():
        ...
        if type(actual) is not type(expected_value) or actual != expected_value:
            raise ValueError("Step 240 authorization binding is invalid")
...
def _validate_evidence(value: object) -> tuple[dict[str, Any], dict[str, Any], tuple[dict[str, Any], ...]]:
    evidence = _exact_object(value, _EVIDENCE_KEYS, "Step 240 evidence")
    ...
    for name, expected in constants.items():
        ...
        if type(actual) is not type(expected) or actual != expected:
            raise ValueError("Step 240 evidence binding is invalid")
    return (
...
class WindowsHostSupportDispositionReceipt:
    receipt_type: str
    schema_version: int
    source: str
    scope: str
    status: str
    policy_as_of: str
    approved_step239_accepted_state_manifest_sha256: str
    approved_step239_package_manifest_sha256: str
    approved_step239_canonical_receipt_sha256: str
    ...
    exact_step239_chain_bound: bool
...
def _validate_receipt(receipt: WindowsHostSupportDispositionReceipt) -> None:
    constants: dict[str, object] = {
    ...
    for name, expected in constants.items():
        ...
        if type(actual) is not type(expected) or actual != expected:
            raise WindowsHostSupportDispositionError("Step 240 receipt constant is invalid")
    if not _is_sha256(receipt.authorization_document_sha256) or not _is_sha256(
        receipt.evidence_document_sha256
    ):
        raise WindowsHostSupportDispositionError("Step 240 receipt digest is invalid")
    ...
    if receipt.authorization_document_sha256 != hashlib.sha256(
        ...
        raise WindowsHostSupportDispositionError(
            "Step 240 authorization receipt digest is invalid"
        )
    ...
    try:
        ...
        raise WindowsHostSupportDispositionError(
            "Step 240 receipt inventory is invalid"
        ) from error
    ...
    if receipt.evidence_document_sha256 != hashlib.sha256(
        ...
        raise WindowsHostSupportDispositionError(
            "Step 240 evidence receipt digest is invalid"
        )
    required_true = (
        "exact_step239_chain_bound",
    ...
    if any(type(getattr(receipt, name)) is not bool or not getattr(receipt, name) for name in required_true):
        raise WindowsHostSupportDispositionError("Step 240 required receipt control is invalid")
    if any(type(getattr(receipt, name)) is not bool or getattr(receipt, name) for name in required_false):
        raise WindowsHostSupportDispositionError("Step 240 fail-closed receipt control is invalid")
    dynamic_booleans = (
    ...
    if any(type(getattr(receipt, name)) is not bool for name in dynamic_booleans):
        raise WindowsHostSupportDispositionError("Step 240 receipt Boolean type is invalid")
    if receipt.commercial_esu_security_updates_only != receipt.commercial_esu_any_licensed:
        raise WindowsHostSupportDispositionError("Step 240 ESU receipt correlation is invalid")
    ...
    if (
        ...
        raise WindowsHostSupportDispositionError("Step 240 ESU disposition is invalid")
    ...
    expected_edition_family = _CLIENT_EDITION_SKU.get(
        (
    ...
    if (
        ...
        raise WindowsHostSupportDispositionError(
            "Step 240 host-identity receipt correlation is invalid"
        )
    ...
    if known:
        if (
            ...
            raise WindowsHostSupportDispositionError("Step 240 known-host receipt is invalid")
        ...
        if (
            ...
            raise WindowsHostSupportDispositionError("Step 240 unselected-host receipt is invalid")
    elif (
        ...
        raise WindowsHostSupportDispositionError("Step 240 uncredible-host receipt is invalid")


def prove_windows_host_support_disposition(
    envelope_document: bytes,
) -> WindowsHostSupportDispositionReceipt:
    ...
    try:
        ...
        if frozenset(envelope) != frozenset(("authorization", "evidence")):
            raise ValueError("Step 240 envelope keys are invalid")
        authorization = _validate_authorization(envelope["authorization"])
        evidence = _exact_object(envelope["evidence"], _EVIDENCE_KEYS, "Step 240 evidence")
        ...
        edition_family = _CLIENT_EDITION_SKU.get(edition_key, "unselected")
        ...
        receipt = WindowsHostSupportDispositionReceipt(
            receipt_type=RECEIPT_TYPE,
            schema_version=SCHEMA_VERSION,
            source=SOURCE,
            scope=SCOPE,
            ...
            policy_as_of=POLICY_AS_OF,
            approved_step239_accepted_state_manifest_sha256=(
                authorization["approved_step239_accepted_state_manifest_sha256"]
            ),
            approved_step239_package_manifest_sha256=(
                authorization["approved_step239_package_manifest_sha256"]
            ),
            approved_step239_canonical_receipt_sha256=(
                authorization["approved_step239_canonical_receipt_sha256"]
            ),
            ...
            exact_step239_chain_bound=True,
        ...
        return receipt
    except WindowsHostSupportDispositionError:
        raise
    except (KeyError, TypeError, ValueError, json.JSONDecodeError) as error:
        raise WindowsHostSupportDispositionError(
            "Step 240 Windows host-support disposition proof failed"
        ) from error


def render_windows_host_support_disposition_receipt(
    receipt: WindowsHostSupportDispositionReceipt,
) -> bytes:
    """Render one exact receipt as a canonical ASCII JSON object."""

    if type(receipt) is not WindowsHostSupportDispositionReceipt:
        raise TypeError("exact Step 240 Windows host-support receipt is required")
    _validate_receipt(receipt)
...
def main() -> int:
    ...
    try:
        ...
        receipt = prove_windows_host_support_disposition(envelope)
        sys.stdout.buffer.write(render_windows_host_support_disposition_receipt(receipt) + b"\n")
        return 0
    except Exception:
...
__all__ = [
    ...
    "STEP239_CANONICAL_RECEIPT_SHA256",
    "STEP239_PACKAGE_MANIFEST_SHA256",
    "WindowsHostSupportDispositionError",
    "WindowsHostSupportDispositionReceipt",
    "main",
    "prove_windows_host_support_disposition",
    "render_windows_host_support_disposition_receipt",
]
